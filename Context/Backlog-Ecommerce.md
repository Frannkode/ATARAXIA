...
- **Usa el mismo `db` (neon-http) que el resto de la app**, con `transaction: false` explícito en la config del adaptador de Drizzle de Better Auth. *(Corrección post-revisión: el issue #4747 de better-auth sobre incompatibilidad con `neon-http` era real pero de una versión vieja — 1.3.10/1.3.11 — ya resuelta desde 1.3.12 con este mismo flag por defecto. La versión instalada (1.6.23) ya lo trae; se setea explícito para que quede documentado en el código, no dependiendo de un default que podría cambiar.)*
...
> **Nota de revisión de arquitectura (`/plan-eng-review`):** revisado antes de implementarse. La revisión propia resolvió el bloqueante del DoR (admin único, sin roles) y encontró decisiones de arquitectura (rate limit respaldado en DB, bootstrap del admin vía seed sin ruta de registro pública, patrón proxy.ts + doble chequeo de sesión). Una revisión cruzada independiente (outside voice) encontró **2 hallazgos críticos** que la revisión propia no vio: (1) `requireAdminSession()` solo cubría Server Actions, dejando las páginas admin (Server Components que leen la DB directo) protegidas únicamente por el chequeo liviano de cookie del proxy — el mismo problema de capa única que el sprint busca evitar; y (2) subir imágenes vía Server Action choca con el límite de ~4.5MB de Vercel, resuelto con subida directa firmada al navegador (el patrón que Cloudinary recomienda, no un workaround). También señaló una supuesta incompatibilidad entre Better Auth y `neon-http` (bug real, pero de una versión vieja de better-auth ya resuelta — ver nota en Historia 4.1: la versión instalada funciona con el mismo `db` de siempre, sin conexión separada). El total subió de 22h/17 SP a 29h/23 SP por esto — principalmente Historia 4.1 (bootstrap) e Historia 4.3 (subida firmada + query admin separada).
